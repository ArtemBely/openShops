...
export const AddProject = () => {
    return (React.createElement("div", { className: 'wrap_post_project', style: { display: typeof window != "undefined" && window.location.pathname.split('/').pop() == 'projects' ? 'grid' : 'none' } },
        ...
        React.createElement("div", null,
            ...
            React.createElement("select", { className: 'news_post_input', id: 'select_category' }, categories.map((item, key) => (React.createElement("option", { key: key, value: key == 0 ? '' : item.name, disabled: key == 0 ? true : false }, item.name))))),
        React.createElement("div", null,
