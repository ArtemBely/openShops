...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    let vacancies = await Vacancy.find();
    let team = await Team.find();
    defineLocation == "projects"
      ? (editObject = await Project.findById(req.params.id))
      : defineLocation == "vacancies"
      ? (editObject = await Vacancy.findById(req.params.id))
      : defineLocation == "news"
      ? (editObject = await News.findById(req.params.id))
            ...
            <head>
              <title>Публикация</title>
                   ...
                   <link type="Image/x-icon" href="/ico.ico" rel="icon">
                     <meta name="viewport" content="width=device-width, initial-scale=1">
                       <script src='../../bundles/bundle.js' defer></script>
                       <script>window.__INITIAL_INFO__ = ${serialize(
                         editObject
                       )}</script>
                       <script>window.__INITIAL_PROJECTS__ = ${serialize(
                         projects
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      actualTeam.noExchangeFile =
        typeof fileName != "undefined" && fileName != null
          ? fileName
          : actualTeam.noExchangeFile;
...
router.post(
  "/projects",
  isLogin,
  upload.fields([{ name: "noExchangeFile" }, { name: "arrayFiles" }]),
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      var newProject: any = new Project({
        ...
        tag: tag,
        mainArray: [
          typeof mainName == "string" ? [mainName] : mainName,
          typeof mainDescription == "string"
            ? [mainDescription]
            : mainDescription,
        ],
        technicalArray: [
          typeof technicalTitle == "string" ? [technicalTitle] : technicalTitle,
          typeof technicalDescription == "string"
            ? [technicalDescription]
            : technicalDescription,
        ],
        descriptionArray: [
          typeof descriptionTitle == "string"
            ? [descriptionTitle]
            : descriptionTitle,
          typeof descriptionTxt == "string" ? [descriptionTxt] : descriptionTxt,
        ],
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (typeof descriptionTitle != "string")
        ...
        actualProject.descriptionArray = [[descriptionTitle], [descriptionTxt]];
      actualProject.noExchangeFile =
        typeof singleFile != "undefined" &&
        singleFile != null &&
        singleFile.length > 1
          ? singleFile
          : actualProject.noExchangeFile;
      actualProject.arrayOfFiles =
        typeof actualProject.arrayOfFiles != "undefined" &&
        actualProject.arrayOfFiles != null
          ? [...actualProject.arrayOfFiles, ...arrayOfImages]
          : arrayOfImages;
...
router.get(
  "/projects/delete/:id/:pathParam1",
  isLogin,
  async (req: Request, res: Response) => {
    let project = await Project.findOne({ _id: req.params.id });
    let index: number | any = project?.arrayOfFiles.findIndex(
      (item: any) => item == req.params.pathParam1
    );
    try {
